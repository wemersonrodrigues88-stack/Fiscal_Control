...
function renderCarteirasCompleta(){
 ...
 const lista=lojas.filter(l=>{
   ...
   if(analista&&String(l.analista||'')!==analista)return false;
   return true;
 });
...
function init(){
 ...
 ['storeSearch','storeUF','storeStatus','storeAnalyst'].forEach(id=>{const el=document.getElementById(id);if(el&&!el.__fcStoreFix){el.addEventListener('input',renderCarteirasCompleta);el.addEventListener('change',renderCarteirasCompleta);el.__fcStoreFix=true}});
