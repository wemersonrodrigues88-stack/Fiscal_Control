...
function analyst(){const u=user();if(u)return u.perfil==='Analista';const p=read('fc_equipe',[]).find(x=>x&&x.nome===name());return !!p&&p.funcao==='Analista'}
...
function render(){if(!analyst())return;const page=document.getElementById('page-dashboard');if(!page)return;const main=page.querySelector('.grid.two');if(!main)return;
 const old=main.querySelector('#fc-analyst-progress-card');
 const cards=[...main.children];
 const obligation=cards.find(c=>c.querySelector('.cardTitle h3')?.textContent.trim()==='Obrigações por estado');
 if(obligation) obligation.remove();
 let card=old;if(!card){card=document.createElement('div');card.id='fc-analyst-progress-card';card.className='card pad';}
 const ss=stores(),e=execs(),n=ss.length;
 card.innerHTML='<div class="cardTitle"><h3>Progresso por imposto</h3><span class="badge blue">'+n+' lojas</span></div><div id="fc-analyst-progress-rows">'+TAXES.map(t=>{const done=ss.filter(l=>{const s=e[String(l.id)+'|'+t]?.status;return s==='Finalizada'||s==='Finalizado'}).length;const pct=n?Math.round(done/n*100):0;return '<div style="display:grid;grid-template-columns:105px 1fr 48px;gap:9px;align-items:center;margin:12px 0;font-size:11px"><b>'+esc(t)+'</b><div style="height:9px;background:#edf1f6;border-radius:99px;overflow:hidden"><span style="display:block;height:100%;width:'+pct+'%;background:#1769e0;border-radius:99px"></span></div><b style="text-align:right">'+done+'/'+n+'</b></div>'}).join('')+'</div>';
 if(card.parentNode!==main)main.appendChild(card);main.style.gridTemplateColumns='1.3fr';
}
