...
(function(){
  ...
  function lockStoreAssignment(){
    ...
    if(!select)return;
    const lojaAtual=(()=>{try{const lojas=JSON.parse(localStorage.getItem('fc_lojas')||'[]');const numero=document.getElementById('sNumero')?.value;return Array.isArray(lojas)?lojas.find(x=>String(x.numero)===String(numero)):null}catch(e){return null}})();
    const lockedValue=String(lojaAtual?.analista||select.value||'');
    select.value=lockedValue;select.disabled=true;select.title='Somente o coordenador ou a gerente pode alterar a carteira.';select.setAttribute('aria-disabled','true');select.style.background='#f3f6fa';select.style.cursor='not-allowed';
    let note=field.querySelector('.fc-analyst-lock-note');
    if(!note){note=document.createElement('small');note.className='fc-analyst-lock-note';note.style.cssText='display:block;margin-top:4px;color:#687589;font-size:10px';note.textContent='Carteira bloqueada para analistas. Alteração exclusiva da coordenação/gerência.';field.appendChild(note)}
    const saveBtn=document.getElementById('modalSave');
    if(saveBtn&&!saveBtn.dataset.fcAssignmentLocked){
      saveBtn.dataset.fcAssignmentLocked='1';
      const original=saveBtn.onclick;
      saveBtn.onclick=function(){select.value=lockedValue;if(typeof original==='function')return original.apply(this,arguments)};
    }
  }
  ...
  function installGuards(){
    ...
    if(typeof window.openStore==='function'){
      ...
      window.openStore=function(id){
        if(isAnalyst()&&!id){toast('Analistas não podem cadastrar novas lojas. A carteira é definida pela coordenação/gerência.');return;}
        const result=oldOpenStore.apply(this,arguments);setTimeout(lockStoreAssignment,0);return result;
      };
    }
    if(typeof window.openAssign==='function'){
      const oldOpenAssign=window.openAssign;
      window.openAssign=function(){if(isAnalyst()){toast('Analistas não podem trocar a carteira de lojas.');return}return oldOpenAssign.apply(this,arguments)};
    }
    window.__fcAnalystAssignmentGuard=true;
  }

  function apply(){cleanup();installGuards();lockStoreAssignment()}
  function init(){apply();setInterval(apply,500);new MutationObserver(()=>setTimeout(apply,20)).observe(document.body,{childList:true,subtree:true})}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
})();

/* ACOMPANHAMENTO-GESTAO-CONSOLIDADO: camada única de acompanhamento gerencial. */
(function(){
  function load(){
...
(function(){var s=document.createElement("script");s.src="apuracoes-fluxo-v2.js?v=4";document.body.appendChild(s);})();
