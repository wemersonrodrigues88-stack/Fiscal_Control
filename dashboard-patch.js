(function(){
  ...
  const TAXES=[
    {key:'ICMS',label:'ICMS'},
    {key:'PIS/COFINS',label:'PIS/COFINS'},
    {key:'ISS',label:'ISS'},
  ...
  function ensureStyle(){
    ...
    const s=document.createElement('style');s.id='fc-acomp-style';s.textContent=`
      .fc-acomp-actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:10px}
      ...
      .fc-acomp-head h2{margin:0;font-size:19px}.fc-acomp-sub{font-size:11px;color:#687589;margin-top:3px}
      .fc-acomp-body{padding:16px 18px}.fc-acomp-toolbar{display:flex;gap:8px;align-items:center;justify-content:space-between;flex-wrap:wrap;margin-bottom:12px}
      .fc-acomp-search{border:1px solid #e1e7ef;border-radius:9px;padding:9px 11px;min-width:240px;font:inherit}
      .fc-acomp-table-wrap{overflow:auto;border:1px solid #e1e7ef;border-radius:12px}
      .fc-acomp-table{width:100%;border-collapse:collapse;min-width:850px}.fc-acomp-table th,.fc-acomp-table td{padding:10px 11px;border-bottom:1px solid #e1e7ef;text-align:left;white-space:nowrap}
      .fc-acomp-table th{font-size:10px;text-transform:uppercase;color:#788498;background:#f8fafc;position:sticky;top:0}.fc-acomp-table td{font-size:12px}
      .fc-status{display:inline-flex;border-radius:99px;padding:5px 8px;font-size:10px;font-weight:850;white-space:nowrap}
      .fc-status.yellowbg{background:#fff3d8;color:#966100}.fc-status.blue{background:#eaf1ff;color:#1b5eb7}.fc-status.greenbg{background:#e5f7f0;color:#087453}.fc-status.gray{background:#edf0f4;color:#667286}
      ...
      @media(max-width:800px){.fc-analyst-grid{grid-template-columns:1fr 1fr}.fc-acomp-search{width:100%;min-width:0}}
  ...
  function openGeneral(){
    ...
    const active=(window.lojas||[]).filter(l=>l&&l.ativo!==false);
    document.getElementById('fc-acomp-sub').textContent=active.length+' lojas · visão consolidada da execução mensal';
    body.innerHTML=`<div class="fc-acomp-toolbar"><div><b>Execução por loja e obrigação</b><div class="fc-acomp-sub">Status: Pendente · Analisando · Finalizado</div></div><input id="fc-geral-search" class="fc-acomp-search" placeholder="🔎 Buscar loja"></div><div class="fc-acomp-table-wrap"><table class="fc-acomp-table"><thead><tr><th>Loja</th><th>ICMS</th><th>PIS/COFINS</th><th>ISS</th><th>SPED</th><th>Fronteiras</th></tr></thead><tbody id="fc-geral-body"></tbody></table></div>`;
    const draw=()=>{const q=(document.getElementById('fc-geral-search').value||'').toLowerCase().trim();const arr=active.filter(l=>`${l.numero??''} ${l.nome??''} ${l.uf??''}`.toLowerCase().includes(q));document.getElementById('fc-geral-body').innerHTML=arr.map(l=>`<tr><td><b>${esc2(l.numero)} · ${esc2(l.nome)}</b><br><small style="color:#687589">${esc2(l.uf)} · ${esc2(l.analista||'Sem carteira')}</small></td>${TAXES.map(t=>{const s=statusOf(l,t);return `<td><span class="fc-status ${s.cls}">${s.label}</span></td>`}).join('')}</tr>`).join('')||'<tr><td colspan="6" class="fc-empty">Nenhuma loja encontrada.</td></tr>'};
  ...
  function openIndividual(){
    ...
    const analysts=(window.equipe||[]).filter(e=>e.funcao==='Analista');
    document.getElementById('fc-acomp-sub').textContent='Carteira por analista · lojas e impostos sob responsabilidade de cada um';
    body.innerHTML=`<div class="fc-analyst-grid" id="fc-analyst-grid"></div><div id="fc-individual-content"></div>`;
  ...
  function drawAnalyst(a){
    ...
    document.getElementById('fc-individual-content').innerHTML=`<div class="fc-acomp-toolbar"><div><b>${esc2(a.nome)}</b><div class="fc-acomp-sub">${esc2(a.nivel)} · ${stores.length} loja(s) na carteira</div></div></div><div class="fc-acomp-table-wrap"><table class="fc-acomp-table"><thead><tr><th>Loja</th><th>ICMS</th><th>PIS/COFINS</th><th>ISS</th><th>SPED</th><th>Fronteiras</th></tr></thead><tbody>${stores.map(l=>`<tr><td><b>${esc2(l.numero)} · ${esc2(l.nome)}</b><br><small style="color:#687589">${esc2(l.uf)}</small></td>${TAXES.map(t=>{const s=statusOf(l,t);return `<td><span class="fc-status ${s.cls}">${s.label}</span></td>`}).join('')}</tr>`).join('')||'<tr><td colspan="6" class="fc-empty">Nenhuma loja vinculada a este analista.</td></tr>'}</tbody></table></div>`;
  }
  function closeModal(){const m=document.getElementById('fc-acomp-modal');if(m)m.classList.remove('show')}
  function patchRender(){
    if(typeof window.render==='function'&&!window.__fcAcompRender){
      const original=window.render;
      window.render=function(){original.apply(this,arguments);setTimeout(ensureUI,0)};
